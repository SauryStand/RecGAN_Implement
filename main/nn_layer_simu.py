...
class Encoder(nn.Module):
    def __init__(self, bsize, embed_dim, encod_dim, model, drop_encoder=0):
        ...
        if self.model=='LSTM':
            self.enc_lstm = nn.LSTM(self.word_emb_dim, self.enc_lstm_dim, 1, batch_first = True, bidirectional=False, dropout=self.dpout_model)
        elif self.model=='GRU':
            self.enc_lstm = nn.GRU(self.word_emb_dim, self.enc_lstm_dim, 1, batch_first = True, bidirectional=False, dropout=self.dpout_model)
        else:
            self.enc_lstm = nn.RNN(self.word_emb_dim, self.enc_lstm_dim, 1, batch_first = True, bidirectional=False, dropout=self.dpout_model, nonlinearity='relu')
